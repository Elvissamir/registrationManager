<template>
  <div>
         <Layout :title="'Editar Profesor'">
            <Container :width="'w-4/12'">
                <div class="flex flex-row text-2xl">
                    <Title>Editar Profesor: {{ teacher.first_name }} {{ teacher.last_name }}</Title>
                </div>
                ...
                <div class="flex mt-4">
                    <form class="w-full" @submit.prevent="submit">
                        ...
                        <div class="flex flex-col mt-3">
                            <label class="block text-gray-700 text-base font-bold mb-2" for="last_name">CI</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" id="ci" v-model="form.ci" />
                            <div class="text-red-500 text-xs italic" v-if="errors.ci">{{ errors.ci }}</div>
                        </div>

                        <div class="flex flex-col mt-3">
                            <label class="block text-gray-700 text-base font-bold mb-2" for="age">Edad </label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="number" id="age" v-model="form.age" />
...
export default {
    ...
    setup(props) {
        ...
        const form = ref({
            ...
            ci: props.teacher.ci,
            phone_mobile: props.teacher.phone_mobile,
            phone_house: props.teacher.phone_house,
        })

        function submit() {
            
            const url = `/teachers/${props.teacher.id}`;
